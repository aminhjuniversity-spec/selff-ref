"""
Path D: Full Dataset Violation Comparison
Runs self-refine evaluation on ENTIRE test sets for all datasets.

This generates the comprehensive results needed for Path D paper.
"""
...
def evaluate_full_dataset(dataset, split=None, data_path=None):
    """
    ...
    if split is not None:
        output_file = f"{output_dir}/{dataset}_split_{split}_full_results.json"
    else:
        output_file = f"{output_dir}/{dataset}_full_results.json"
...
def evaluate_all_datasets():
    ...
    for split in range(5):
        results = evaluate_full_dataset(dataset='PH2', split=split)
        all_results[f'PH2_split_{split}'] = results['summary']
    
    # Derm7pt
    print("\n[DATASET 2/3] Derm7pt")
    results = evaluate_full_dataset(dataset='Derm7pt', split=None)
    all_results['Derm7pt'] = results['summary']
    
    # HAM10000
    print("\n[DATASET 3/3] HAM10000")
    results = evaluate_full_dataset(dataset='HAM10000', split=None)
    all_results['HAM10000'] = results['summary']
    
    # Save consolidated results
    output_file = "results/path_d_full_evaluation/CONSOLIDATED_RESULTS.json"
    with open(output_file, 'w') as f:
    ...
    print("  3. Write Results section (Section 4)")
